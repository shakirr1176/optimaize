            ...
            <div class="w-full px-4 mt-5">
                <div class="tabeArea rounded-xl overflow-hidden">
                        ...
                        <div class="flex items-center space-x-2">
                            ...
                            <a class="bg-danger hover:bg-opacity-90 text-white lara-btn confirmation" data-form-method="put" data-alert="{{ 'Are you sure?' }}" data-form-id="close-{{ $ticket->id }}" href="{{ route('admin.tickets.close', ['ticket' => $ticket->id]) }}">
                                {{ __('Close Ticket') }}
                            </a>
                            @endif

                            @if (has_permission('admin.tickets.resolve'))
                            <a class="bg-lara-blue hover:bg-opacity-90 text-white lara-btn confirmation" data-form-method="put" data-alert="{{ 'Are you sure?' }}" data-form-id="resolve-{{ $ticket->id }}" href="{{ route('admin.tickets.resolve', ['ticket' => $ticket->id]) }}">
                                {{ __('Resolve Ticket') }}
                            </a>
                            @endif
                        </div>
                        ...
                        <div>
                            <ul class="flex flex-wrap items-center -mx-4">
                                ...
                                <li class="flex items-center px-4 my-1">
                                    <h5 class="dark:text-white text-lara-whiteGray font-14 font-medium mr-4">{{ $ticket->status === TicketStatusEnum::RESOLVED->value ? __('Resolved By') : __('Closed By') }} :</h5>
                                    <p class=" font-14 dark:text-white text-lara-whiteGray">{{ $ticket->assignedUser?->fullName }}</p>
                                </li>
                                @endif
                                <li class="flex items-center px-4 my-1">
                        ...
                        <div class="flex flex-wrap -mx-3">
                            ...
                            <div class="flex-1 px-3">
                                <div class="flex items-center space-x-4 mt-3 md:mt-0">
                                    <h4 class="font-18 dark:text-white text-lara-whiteGray">{{ $ticket->user->fullName }}</h4>
                                    <span class="text-black-50 font-16">{{ $ticket->created_at !== null ? $ticket->created_at->diffForHumans() :'' }}</span>
                                </div>
                                <div class="mt-3">
                                    ...
                                    <a href="{{ route('admin.tickets.attachment.download', ['ticket' => $ticket->id,'fileName' => $ticket->attachment]) }}" class="mt-3">
                                        <label>
                                            <span class="inline-flex items-center space-x-2 text-success cursor-pointer">
                                ...
                                <div class="flex flex-wrap -mx-1 mt-7 border-l-2 border-dashed ml-1.5">
                                    ...
                                    <div class="flex-1 px-1 pl-4">
                                        <div class="flex items-center space-x-4 mt-3 md:mt-0">
                                            <h4 class="font-18 dark:text-white text-lara-whiteGray capitalize">{{ $comment->user->fullName }}</h4>
                                            <span class="text-black-50 font-16">{{ $comment->created_at !== null ? $comment->created_at->diffForHumans() : '' }}</span>
                                        </div>
                                        <div class="mt-3">
                                            <p class="capitalize font-14 dark:text-white text-lara-whiteGray">
                                                {{ $comment->content }}
                                            </p>
                                            @if($comment->attachment)
                                            <a href="{{ route('admin.tickets.attachment.download', ['ticket' => $ticket->id,'fileName' => $comment->attachment]) }}" class="mt-3">
                                                <label>
                                                    <span class="inline-flex items-center space-x-2 text-success cursor-pointer">
                                                        @svg('heroicon-s-paper-clip', 'w-4')
                                                        <span class="pt-1 text-sm tracking-wider">
                                ...
                                @if(!in_array($ticket->status,[TicketStatusEnum::RESOLVED->value, TicketStatusEnum::CLOSED->value]) && has_permission('admin.tickets.comment.store'))
                                <form action="{{ route('admin.tickets.comment.store',$ticket->id) }}" method="post" enctype="multipart/form-data">
                                    @csrf
                                    <div class="flex flex-wrap -mx-1 mt-7">
                                        <div class="w-full px-1">
                                            <textarea rows="5" class="outline-none px-4 py-4 w-full font-16 text-black-50 dark:bg-lara-darkBlack bg-white border dark:border-gray-600 rounded-md" name="content" placeholder="{{ __('Type your message here...') }}"></textarea>
                                            <p class="text-sm text-red-600 mt-2">{{ $errors->first('content') }}</p>
                                        </div>
                                    </div>
                                    <div class="flex flex-wrap -mx-1 mt-7">
                                        <div class="w-full px-1 parentImgUploadDiv">
                                            <div class="group relative mt-4">
                                                <div class="drop-area flex flex-col justify-center items-center w-full h-32 border-2 dark:border-gray-600 border-gray-300 border-dashed cursor-pointer">
                                            ...
                                            <input class="uploadImgBtn" type="file" hidden="" name="attachment">
                                            <p class="text-sm text-red-600 mt-2">{{ $errors->first('attachment') }}</p>
                                        </div>
                                    </div>
                                    <div class="w-full mt-7">
                                        <div class="flex items-center space-x-3">
                                            <button class="bg-lara-blue hover:bg-opacity-90 text-white lara-btn" type="submit">
                                                {{ __('Submit') }}
                                            </button>
                                            <button class="bg-danger hover:bg-opacity-90 text-white lara-btn" type="reset">
                                                {{ __('Reset') }}
                                            </button>
                                        </div>
                                    </div>

...
<x-app-layout>
    ...
    <form class="w-full" method="post" action="{{ route('admin.roles.update', $role->slug) }}">
        ...
        @foreach ($routes as $name => $routeGroups)
            ...
            <div class="role-permission rounded-xl overflow-hidden mb-4 2xl:mb-6">
                <div class="dark:bg-dark-optm-gray-300 bg-optm-gray-300 py-3 px-6 border dark:border-lara-darkBlack">
                        ...
                        <div class="main-input lg:w-1/5 w-full px-4">
                            <div class="flex items-center space-x-3">
                                ...
                                <div class="cursor-pointer rolebutton">
                                    <h2 class="font-16 dark:text-white text-dark_2 dark:text-branco-sujo font-medium">
                                        {{ $str->title($str->replace('_', ' ', $name)) }}</h2>
                                    <div class="dark:text-white text-dark_2 dark:text-branco-sujo flex items-center space-x-3">
                        ...
                        <div class="lg:w-4/5 w-full px-4">
                            <div class="row">
                                <div class="sub-col-input md:w-1/4 px-4 mt-5 lg:mt-0 w-1/4">
                                    <p class="dark:text-white text-dark_2 dark:text-branco-sujo font-14 font-medium mb-1 text-center">
                                        {{ __('Read Access') }}</p>
                                    <div class="flex justify-center">
                                        <label class="switch relative inline-block">
                                            <input class="opacity-0 w-0 h-0" type="checkbox">
                                            <span
                                                class="dark:text-white text-dark_2 dark:text-branco-sujo flex px-3 uppercase text-base items-center justify-between slider round absolute cursor-pointer inset-0 bg-black-30 duration-300">
                                            </span>
                                        </label>
                                    </div>
                                </div>
                ...
                <div class="roleDrop hidden">
                    ...
                    @foreach ($routeGroups as $groupName => $permissionLists)
                        <div class="sub-row-input-parent dark:bg-dark-optm-gray-300 bg-optm-gray-300 py-5 px-6">
                            @php $allSubModules = true @endphp
                            @foreach ($permissionLists as $permissionName => $routeList)
                                @php
                                    if (!isset($role->permissions[$name][$groupName]) || !in_array($permissionName, $role->permissions[$name][$groupName])) {
                            ...
                            <div class="row">
                                <div class="sub-row-input lg:w-1/5 w-full px-4">
                                    <div class="flex items-center space-x-3">
                                        ...
                                        <p class="font-14 text-dark_2 dark:text-branco-sujo font-medium">

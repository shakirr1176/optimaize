    ...
    <div class="mt-6 2xl:mt-10 w-full">
        <div class="row">
            <div class="md:w-1/3 w-full px-4 mb-6 md:mb-0">
                <div class="dark:bg-dark-optm-gray-300 bg-optm-gray-300 rounded 2xl:py-4 md:py-2 py-2 px-6">
                    <p class="font-16 text-center lg:text-left dark:text-white text-dark_1 font-medium">
                        {{ __('Settings') }}</p>
                </div>
                <ul class="mt-3">
                    @foreach ($settings['settingSections'] as $key => $value)
                        @if (!empty($value['sub-settings']))
                            <li
                                class="{{ $type == $key ? 'group dark:bg-dark_2 bg-gray-400/80 text-white' : 'dark:bg-optm-gray-300 bg-gray-400/80 group hover:bg-gray-400/80 dark:text-white text-white' }} flex items-center justify-between duration-300 2xl:py-4 md:py-2 py-1.5 px-5 rounded-t mt-1.5 first:mt-0">
                            ...
                            <ul class="dark:text-white text-dark_1 dark:bg-dark-optm-gray-300 bg-optm-gray-50">
                                @foreach ($value['sub-settings'] as $itemKey => $item)
                                    <li class="">
                                        <a href="{{ route('admin.settings.edit', ['type' => $key, 'sub_type' => $item]) }}"
                                            class="block transform px-6 2xl:py-3 py-2 group font-14 font-medium duration-300 {{ $sub_type == $item ? 'dark:bg-dark_2 bg-optm-gray-300 dark:bg-opacity-40 bg-opacity-40' : 'dark:hover:bg-dark_2 hover:bg-gray-400 dark:hover:bg-opacity-40 hover:bg-opacity-40' }}">{{ __(ucwords(preg_replace('/[-_]+/', ' ', $item))) }}</a>
                                    </li>
                                @endforeach
                            </ul>
                        @endif
            ...
            <div class="md:w-2/3 w-full px-4 mt-3 md:mt-0">
                ...
                <div class="dark:bg-dark-optm-gray-300 bg-optm-gray-50 rounded p-6 sm:p-8 2xl:p-14 mt-3">

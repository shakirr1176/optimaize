<x-app-layout>
    ...
    <x-datatable :data="$announcements"></x-datatable>

    <!-- modal start  -->
    <x-modal class="addModal" title="{{ __('Create Announcement') }}">
        <x-slot name="inputs">
            @include('admin.announcements._create-form')
        </x-slot>
    ...
    <x-modal class="editModal" title="{{ __('Update Announcements') }}">
        <x-slot name="inputs">
            @include('admin.announcements._update-form')
        </x-slot>
    ...
    <x-section name="scripts">
        ...
        <script>
            ...
            document.querySelector("#createSubmitBtn").addEventListener("click", function(e) {
                ...
                let route = "{{ route('admin.announcements.store') }}";
                let method = "POST";
            ...
            document.querySelector("#updateSubmitBtn").addEventListener("click", function(e) {
                e.preventDefault();
                let form = document.querySelector(".editModal").children[0];
                let route = "{{ route('admin.announcements.update', ':updateId') }}".replace(":updateId", updateId);
                let method = "PUT";

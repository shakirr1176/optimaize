    ...
    @foreach ($options as $key => $value)
        <div class="flex items-center">
            ...
            <label class="cursor-pointer text-sm flex items-center" for="Approve{{ $id.$loop->iteration }}">
                ...
                <span class="2xl:w-4 w-2.5 2xl:h-4 h-2.5 duration-300 dark:bg-lara-blue bg-lara-gray-400 rounded-full block"></span>
                </span>
                <span class="block font-14 font-medium text-dark_2/80 capitalize">{{ $value }}</span>
            </label>
        </div>
    @endforeach
</div>

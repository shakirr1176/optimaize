@if (empty($menu['children']))
    <x-sidebar.li>
        @if ($menu['icon'])
            ...
            class="drop-down-header {{ $menu['active'] ? 'active-menu active-on' : '' }} mt-1 flex items-center justify-between px-6 py-3 font-light capitalize text-lara-gray-400 dark:text-white duration-200">
        ...
        <span class="block curve-svg w-2.5 h-0.5 absolute top-5 -translate-y-1/2 -left-0.5">
            ...
            class="drop-down-header {{ $menu['active'] ? 'active-menu active-on' : '' }} group-[.expand-sidebar]:ml-3.5 mt-1 flex items-center !justify-between py-3 pl-3 pr-18px font-light capitalize text-lara-gray-400 dark:text-white duration-200">
                <span
    ...
    <x-sidebar.li>
        @if ($menu['icon'])
            <a href="javascript:" title="{{ __($menu['title']) }}"
                class="drop-down-header main-header mt-1 flex items-center px-6 py-3 font-light capitalize text-lara-gray-400 dark:text-white duration-200">
                <span class="flex items-center font-medium">
                    <span class="menu-icon flex flex-shrink-0 items-center justify-center">
                        @svg($menu['icon'], 'w-full')
                    </span>
        ...
        <span class="block curve-svg w-2.5 h-0.5 absolute top-5 -translate-y-1/2 -left-0.5">
            ...
            <a href="javascript:" title="{{ __($menu['title']) }}"
                class="drop-down-header group-[.expand-sidebar]:ml-3.5 mt-1 flex items-center justify-between py-3 pl-3 pr-18px font-light capitalize text-lara-gray-400 dark:text-white duration-200">
                <span
                    class="font-14 block w-32 xl:w-40 2xl:w-44 items-center overflow-hidden text-ellipsis whitespace-nowrap pr-2">
                    {{ __($menu['title']) }}
                </span>
